...
export default function BudgetCard({
  ...
  return (
    <Card className={classNames.join(' ')}>
        <Card.Body>
            <Card.Title className='d-flex justify-content-between align-items-basline fw-normal mb-3'>
                <div className='me-2'>{name}</div>
                <div className='d-flex align-items-baseline'>{cucrrencyFormatter.format(amount)}
                  {max && (<span className='text-muted fs-6 ms-1'>
                     / {cucrrencyFormatter.format(max)}
                  </span>)}
